# Source Classification

Understanding the distinction between original records, derivative sources, and authored works.

## Why Classification Matters

The reliability of genealogical evidence begins with understanding what kind of source you're using. Each time information is copied or interpreted, errors can be introduced.

## Original Sources

### Definition
An **original source** is a record that:

- Contains information in its first recorded form
- Was created at or near the time of the event
- Exists in its initial physical form or authorized reproduction

### Characteristics
- Handwritten or typed at time of creation
- Official seals, signatures, or authentication
- Physical evidence of age and authenticity
- May contain corrections made at time of creation

### Common Misconceptions
- A **photocopy of an original** is still accessing the original
- A **microfilm image** of an original preserves original status
- A **digital scan** of an original maintains original quality
- FamilySearch or Ancestry **images** of originals are originals

## Derivative Sources

### Definition
A **derivative source** is a record that:

- Is copied, transcribed, abstracted, or extracted from another source
- May introduce copying errors or interpretation
- Is removed by one or more steps from the original

### Types of Derivatives

#### Transcriptions
Full word-for-word copies of originals.

**Advantages**:

- Preserve full text
- Searchable if indexed

**Risks**:

- Misread handwriting
- Typographical errors
- Omitted words or phrases

#### Abstracts
Condensed versions containing selected information.

**Advantages**:

- Easier to use
- Often compiled into books

**Risks**:

- Important details omitted
- Abstractor's interpretation imposed
- Context lost

#### Extracts
Selected portions copied verbatim.

**Advantages**:

- Key information preserved exactly

**Risks**:

- Surrounding context missing
- Selection bias

#### Indexes
Lists with minimal identifying information.

**Advantages**:

- Finding aids for originals

**Risks**:

- Names often misspelled
- Details stripped away
- Many omit entries

### Derivative of a Derivative
Each generation of copying introduces potential errors:

```
Original Register → Transcription → Index → Your Notes
        ↓               ↓            ↓          ↓
      Best          Good-ish      Risky    Most risky
```

## Authored Works

### Definition
An **authored work** is a source that:

- Compiles, analyzes, or interprets information from multiple sources
- Reflects the author's conclusions
- May or may not cite underlying sources

### Types

#### Compiled Genealogies
- Family histories
- Lineage books
- Published genealogies

#### Narrative Histories
- Local histories with family sections
- County biographical volumes
- DAR/SAR applications

#### Reference Works
- Encyclopedias
- Biographical dictionaries
- Historical compilations

### Reliability Factors

**More Reliable When**:

- Sources are cited for each fact
- Author had access to original records
- Work has been peer-reviewed
- Author is a credentialed researcher

**Less Reliable When**:

- No sources cited
- Based on family tradition
- Published vanity genealogy
- Multiple generations without documentation

### Example Analysis
```
"History of Muskingum County" (1882)

Type: Authored work (compiled county history)
Reliability: Variable
- Some sections based on original records
- Family sections often from submitted sketches
- Advertisements paid for inclusion
- Must verify each claim independently
```

## Chain of Custody

Understanding how a source reached you:

```
Original Event
     ↓
Original Record (e.g., church register)
     ↓
Derivative (e.g., published transcription)
     ↓
Your Copy (e.g., photocopy or digital download)
```

**Document each step**:

- What was the original record?
- What derivative did you use?
- Where did you obtain it?
- What are the potential sources of error?

## Quality Indicators

### Signs of a Reliable Derivative
- Published by reputable society (DAR, NEHGS, state archives)
- Includes images of originals alongside transcriptions
- Notes illegible portions honestly
- Provides repository information for originals

### Red Flags
- No identification of original source
- "Family tradition" or "family Bible" without images
- Dates too precise for the era
- Details that seem too convenient
- Copied from another genealogy without verification
