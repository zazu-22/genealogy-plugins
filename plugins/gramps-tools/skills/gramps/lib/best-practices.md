...
All Gramps data modifications should use GrampsAPIClient. See
[interactive-usage.md](interactive-usage.md) for full patterns (instantiation,
dry-run, error handling, troubleshooting) and [web-api.md](web-api.md) for
endpoint reference.

Always test migrations with dry-run mode first. See production usage in
`~/code/gramps_plugins/tools/census_migration/`.

### Undo Support

API changes can be undone via transaction history — see web-api.md for
